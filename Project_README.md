...
Balanced white space and card‑style visuals
...
Avg = DIVIDE(SUM(Scores[Score]),DISTINCTCOUNT(Scores[Subject]))*100

Attendance = DIVIDE(CALCULATE(COUNTROWS(Attendance),Attendance[Status] = "Present"),COUNTROWS(Attendance))*100

Behavior Count = DISTINCTCOUNT(Behavior[BehaviorType])
